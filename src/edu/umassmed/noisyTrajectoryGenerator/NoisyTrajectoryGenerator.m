...
function NoisyTrajectoryGenerator(snrFrom, snrTo)
	...
	%subFolder = strcat('F:');
    %folder = strcat(subFolder, pathSep, '2017-02-17_FullSimu_LD3_Test3');
	%distriFolder = strcat(subFolder, pathSep, '2014-01-24_Image for bias and sigma estimation');
    supFolder = strcat(pathSep, 'home', pathSep, 'ar45w', pathSep, 'matlab');
	folder = strcat(supFolder, pathSep, '2018-05-10_FullSimu_LD10_NoTracks');
	distriFolder = strcat(supFolder, pathSep, '2018-04-13_Images_BiasAndSigmaEstimation_NewJavaAlgo_NewP2P');
    ...
    for i = snrFrom:snrTo
        ...
        reportFile = strcat(folder, pathSep, sprintf('Report_%4.6f.txt', SNR));
        ...
        for ii = 1:l_L
            ...
            for iii = 1:l_SMSS
                ...
                for iiii = 1:l_D
                    ...
                    s = sprintf('Elapsed time is %f seconds', toc(t1));
                    fprintf(fidR, s);
                    fprintf(fidR, '\n');
                    disp(s);
                end
